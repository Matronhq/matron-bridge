...
- `GET $BASE/items?convo=<id>&state=open` — list items for one conversation. `state` is `open` (default) or `closed` — there is no `any`; omit `state` to get both. Add `kind=task|question|decision`, `awaiting=user|agent`, or `label=<name>` to narrow further. Omit `convo` to list across every conversation of this user instead — there is no "current conversation" default here (see below).
- `POST $BASE/items` — file one: `{"kind":"question"|"decision"|"task","title":"...","body":"...","convo_id":"<id>"}` (optional `labels`, `links`, `awaiting`, `supersedes`, and exactly one of `position` (`"top"`/`"bottom"`), `after`, or `before` an existing item id). `convo_id` is required — the journal does not fill it in for an HTTP caller the way the MCP tools do for a Claude Code session.
