...
export class TranscriptTail extends EventEmitter {
  ...
  _tick() {
    ...
    // openSync needs its own guard: the file can be deleted or replaced
    // between the statSync above and this open (especially on session
    // resume / restart). Throwing from _tick would crash the bridge
    // because the caller is a setInterval with no outer handler.
    let fd;
    try {
