...
export function createMissionsHandlers({ sessions, journalConvoIdFor, client }) {
  ...
  async function resolveMission(session, convoId) {
    ...
    if (r.status !== 200 || !Array.isArray(r.data?.missions)) {
      ...
      // moment the journal is back. Surface the outage instead; only a
      // clean 200 with an empty list means there is nothing to find.
      return { err: passthroughCollection(r) };
    }
