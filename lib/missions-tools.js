  ...
  return {
    ...
    async get(data) {
      ...
      if (id === undefined) {
        ...
        if (!resolved) return { status: 404, body: { error: NO_MISSION } };
        return remember(session, passthroughCollection(await client.get(resolved)));
      }
      if (!Number.isInteger(id) || id < 1) return bad('num must be a positive integer');
      return passthroughCollection(await client.get(id));
    },
