...
export function createMissionsHandlers({ sessions, journalConvoIdFor, client }) {
  ...
  return {
    ...
    async create(data) {
      ...
      // A fresh create (201; an idempotent replay answers 200) of an
      // unassigned mission has no conversations. A count above zero is a
      // journal that ignored attach:false and joined this conversation to
      // the mission it just made. Only when the response carries the count.
      const joined = r.body?.mission?.conversations;
      if (r.status === 201 && typeof joined === 'number' && joined > 0) {
        return { status: 502, body: { error: attachedAnyway(r.body.mission.num) } };
      }
      return r;
    },
