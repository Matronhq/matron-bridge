...
// than reviving unconditionally on every start.
...
export function createSubagentConvoTracker({ publisher, getParentConvoId, runningStore = null, log = console } = {}) {
  ...
  function ensureChild(agentId, { label, agentType } = {}) {
    ...
    child = {
      ...
      // resume). Sole use is the id-less task_notification rule: a
      // generation-0 run has exactly one incarnation, so an uncorrelated
      // completion is unambiguous. A corrective revival is the
  ...
  // rendered `done` with no recovery path at all.
  function _tryRevive(child, agentId, { incrementGeneration = true } = {}) {
    ...
    // tool_result) is the SAME incarnation, so leaving generation
    // at 0 keeps the never-resumed id-less completion fallback working.
  ...
  function finish(agentId) {
    ...
    // revival (a genuine resume OR the corrective revival), whereas
  ...
  return {
    ...
    noteBackgroundTaskStarted(toolUseId, taskId) {
      try {
        ...
        if (child.taskRef === toolUseId) {
          ...
          //     so it must revive, not be dropped. A live agent
        ...
        //
        // Reject a REPLAYED task_started that would restore a
        ...
        // path) and must be allowed. State alone is not a valid discriminator (an
        // earlier revision rejected these valid corrections).
        child.taskRef = toolUseId;
        _publishStatus(child);
    ...
    // must NOT finish the live resumed run (run N+1).
    //
    ...
    // early by the launch tool_result: that is the same
    // incarnation, so generation must stay put or the id-less completion fallback
    // would wrongly treat the single run as resumed and strand it.
