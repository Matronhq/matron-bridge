...
// The two auto-resume announcements. Same shape as index.js's
// JOURNAL_RESUME_NOTICE, but each says which event is doing the waking —
...
const RESUME_NOTICE_EXPIRED = '⏳ Session was idle — auto-resuming it to report an expired secret request.';
...
export function createSecretRequests({
  ...
  async function tell(record, { text, preview, resumeNotice, undeliverable }) {
    try {
      ...
      if (!session) session = resumeSession(record.roomId, resumeNotice);
      if (!session) {
  ...
  async function expire(record) {
    ...
    await tell(record, {
      ...
      resumeNotice: RESUME_NOTICE_EXPIRED,
      undeliverable: `Couldn't deliver a message to the agent — the session isn't available. The "${record.label}" secret request expired without a submission.`,
    });
  }
