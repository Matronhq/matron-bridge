...
export function createJournalMediaRouter({
  ...
  function depositBatchItem(session, tag, item) {
    const key = `${(session && session.claudeSessionId) || 'unknown'}|${tag.id}`;
    ...
    // replay) must not double its blocks into the prompt.
    if (!entry.items.has(tag.index)) entry.items.set(tag.index, item);
    if (entry.timer) clearTimeout(entry.timer);
    entry.timer = null;
    if (entry.items.size >= entry.expected) {
      batches.delete(key);
      return finalizeBatch(session, entry, { partial: false });
    }
  ...
  async function routeOne(session, media, ctx = {}) {
    ...
    try {
      ...
      if (batch) {
        ...
        // timer) delivers everything as one injection.
        await depositBatchItem(session, batch, {
          blocks,
