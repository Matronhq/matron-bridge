  ...
  async function routeOne(session, media, ctx = {}) {
    try {
      ...
      if (!Array.isArray(blocks) || blocks.length === 0) {
        ...
        // A captioned attachment that produced no blocks never reached claude,
        // so a bare "look at this" with nothing to look at would mislead. Tell
        // the user instead of silently dropping (matches the fetch/transcribe
        // failure notices above).
