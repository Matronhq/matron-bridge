...
export function createTimerStore({ load, save, now, setTimer, clearTimer, onFire, log = () => {} }) {
  ...
  return {
    // Re-arm everything persisted from a previous bridge run. Overdue
    // records (came due while the bridge was down) fire after a short grace
    // rather than synchronously — see OVERDUE_GRACE_MS.
    init() {
      for (const record of timers) arm(record, { minDelay: OVERDUE_GRACE_MS });
