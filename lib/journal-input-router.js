  ...
  function onJournalEvent(frame) {
    try {
      ...
      if (!session && routeSessionlessPickerTap && type === 'prompt_reply' && payload?.target_seq != null
        && isSleepPickerTap(pickerFrames.get(convoId, payload.target_seq), payload?.choice)) {
        ...
        // Claude). Provenance is the same as for resume taps: the choice must
        // be a value the targeted frame itself offered.
        try {
