...
const PICKER_OPTION_ID = /^(?:model|effort|mode|timer)-/;
...
export function createJournalInputConsumer({
  ...
  // journal side, so a room peer configured with the SAME device name as
  // this bridge is indistinguishable from our own echoes — both directions
  // get classified own-echo and the room dies silently. The drop is the
  // correct fail-safe; the silence isn't, so say why once.
