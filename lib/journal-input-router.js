...
// the rest queued, `make_task` files it in the tracker instead of sending it,
// and `cancel` drops just the tapped message. A value missing from this set is
// refused as invalid — so an action added to the card in lib/busy-queue.js must
// be added here too, or every tap on it is a dead end.
const QUEUED_RELEASE_ACTIONS = new Set(['send', 'send_one', 'make_task', 'cancel']);
  ...
  function onJournalEvent(frame) {
    try {
      ...
      if (!session && resumeSessionForConvo) {
        ...
        // tracker exists for. Gated on the SAME turn-worthiness rule the route
        // applies, so a reorder, a field edit or a malformed marker can never
        // respawn a session for a turn that would then be thrown away.
      ...
      if (isItem) {
        ...
        // filed, a close or a reopen. The route builds the synthetic turn and
        // decides what is worth saying — a reorder or a field edit produces
        // nothing there (spec: those never become a turn), which is why this
        // branch does not pre-filter by action.
