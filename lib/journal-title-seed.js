...
// The leading characters of the session id, prefixed to every EARNED title
// (the first-message fallback below, the Gemini rename, resume titles).
// Which box owns a conversation is data clients render as a chip or
// subtitle, but nothing else tells two sessions on the SAME box apart — the
// short went out with the server label in #205 and was missed immediately.
// Seed titles stay bare: at seed time the native session id often doesn't
// exist yet, and the seed-detection in legacySeedTitles stays a closed set
// of label-era forms.
//
// Three characters, not two, since 2026-09: session ids are hex, so two
// characters gave 256 handles and they collided between LIVE rooms on one
// busy box (a `🔗 [94]` link room beside a `[94]` project room). Three gives
// 4,096. Every title published before this carries a two-character short
// for ever — titles only rewrite on rename — so the reader below accepts
// both, and so do the apps (SessionTag.splitTitle, `shortLengths`).
export const SESSION_SHORT_LEN = 3;

// The short for a session id (or a room id when the native id is unknown):
// '' when there is nothing to short. One place, so the title writer and the
// agent-chat announcements can never disagree about a session's handle.
export function sessionShortOf(id) {
  return typeof id === 'string' ? id.trim().slice(0, SESSION_SHORT_LEN) : '';
}

export function withSessionShort(id, title, marker = '') {
  const short = sessionShortOf(id);
...
// SessionTag.splitTitle): two or three alphanumerics in brackets (two for
// every title minted before SESSION_SHORT_LEN went to three, which never
// rewrite), a single space, and a non-empty title after it. Bracketed text
// that misses any of those is ordinary title text and yields nothing, so
// `[WIP] ship it` is never mistaken for a short. Markers are read through,
// not stripped from the caller's view: the short sits BEHIND them.
export function sessionShortFromTitle(raw) {
  ...
  return /^\[([\p{L}\p{N}]{2,3})\] .+$/u.exec(raw)?.[1] || '';
}
