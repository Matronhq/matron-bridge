...
function seedTitleFor(workdir, serverLabel) {
  const base = workdir ? path.basename(path.resolve(workdir)) : '';
  const name = base || 'session';
  return serverLabel ? `${serverLabel}: ${name}` : name;
}
...
export function applyFallbackTitle(session, { serverLabel, updateRoomName, workdir }) {
  ...
  // The bare-basename form is the seed sessions persisted before the label
  // was added — still just a seed, still fair game to replace.
  if (hint !== undefined
      && hint !== seedTitleFor(workdir, serverLabel)
      && hint !== seedTitleFor(workdir)) return false;
...
export async function seedJournalTitle(session, { workdir, serverLabel, incomingHint, persistedHint, reattaching = false, upsertConvo, warn = () => {} }) {
  try {
    ...
    upsertConvo(session, { title: seedTitleFor(workdir, serverLabel) });
    return true;
  } catch (e) {
