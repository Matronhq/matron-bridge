...
// name. The value runs to the next whitespace, and a leading quoted segment
// (which may contain spaces) is consumed whole along with anything glued to
// it. Input that already looks redacted is not trusted: re-redacting an
// `[REDACTED-ENV]` value is idempotent.
const INLINE_ASSIGNMENT_RE = /(?<![A-Za-z0-9_])([A-Za-z_][A-Za-z0-9_]*=)(?:"[^"]*"?|'[^']*'?)?\S*/g;
