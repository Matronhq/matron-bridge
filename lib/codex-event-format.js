...
//
// SECURITY — guards-first: this open-ended routing is only safe
...
// gated on item.completed reopened a secret-egress hole (env dumps
...
function looksLikeRawEnvDump(output) {
  ...
  // an uppercase-only regex let lowercase dumps bypass the guard.
...
// with a fixed marker; an assignment embedded in prose has its value redacted
// too (see INLINE_ASSIGNMENT_RE). Applied to newly forwarded DIAGNOSTIC text
// (top-level error strings, fallback diagnostics) so an env dump under an innocuous env
...
// continuation bytes of a secret value leak past the scrub.
...
function scrubAssignmentValues(text) {
  ...
  return text
    .split(separators)
    .map(entry => entry
      .replace(assignment, '$1$2[REDACTED-ENV]')
      .replace(INLINE_ASSIGNMENT_RE, '$1[REDACTED-ENV]'))
    .join(nulFramed ? '\0' : '\n');
}

// An assignment embedded in prose (`fatal: ALPHA=secret`, `--token=secret`).
// The baseline redactor only recognizes secret-looking NAMES, so an innocuous
// name would carry its value out; diagnostics fail closed and keep only the
// name. An unquoted value runs to the next whitespace. A quoted value is
// redacted to the end of the entry: quote/escape parsing of untrusted text is
// ambiguous, so it fails closed rather than guessing where the value ends.
// Input that already looks redacted is not trusted: re-redacting an
// `[REDACTED-ENV]` value is idempotent.
const INLINE_ASSIGNMENT_RE = /(?<![A-Za-z0-9_])([A-Za-z_][A-Za-z0-9_]*=)(?:["'][\s\S]*|\S*)/g;
...
function allowlistedEvent(event) {
  ...
  if (event?.item && typeof event.item === 'object') {
    ...
    if (itemType === 'command_execution') {
      // GUARDS-FIRST: drop the whole event when the command is an
      ...
      // dump under any envelope is dropped outright).
    ...
    //. Diagnostics degrade safely to the type stub; the
...
//. Values also pass through the shared redactor; arbitrary
...
// The run's terminal failure line. Like the final answer it is the run's
// result, not an intermediate event, so it is exempt from the durable-event
// cap: an answer landing in the last slot must not push the failure reason
// out. Posted at most once per run.
function publishTerminalFailure(ctx, state, payload) {
  if (state.terminalFailurePosted) return;
  state.terminalFailurePosted = true;
  state.durableEvents += 1;
  const options = typeof ctx.runId === 'string' && ctx.runId.length > 0
    ? { idemKey: `${ctx.runId}:failed` }
    : undefined;
  ctx.publisher.publishText(ctx.convoId, payload, ...(options ? [options] : []));
}
...
export function formatAndRoute(event, ctx) {
  ...
  if (!schemaIsSupported(ctx.meta?.schemaVersion)) {
    ...
    if (event?.type === 'turn.failed') {
      // Same terminal policy as the supported path: an answer produced before
      // the failure still lands, then the (fallback-scrubbed) failure event.
      emitDurableFinalAnswer(ctx, state);
      state.unparsed += 1;
      publishTerminalFailure(ctx, state, { body: stringify(event), from: 'assistant' });
      state.terminalSeen = true;
      ctx.publisher.publishActivity(ctx.convoId, 'idle');
      return state;
    }
  ...
  // Known codex exec --json protocol events that used to fall to
  ...
  if (event?.type === 'turn.failed') {
    ...
    emitDurableFinalAnswer(ctx, state);
    const span = diagnosticSpan(diagnosticOf(event));
    publishTerminalFailure(ctx, state, {
  ...
  if (event?.type === 'item.started') {
    ...
    if (event.item?.type === 'command_execution') {
      ...
      // GENERIC item.started fallback: any recognized-shape item
  ...
  if (event?.type === 'item.completed' && typeof event.item?.type === 'string') {
    // GENERIC item.completed fallback: any recognized-shape item
    ...
    // failed tool as succeeded. A neutral text line makes no
    // success/failure claim. (Honoring a real terminal status end-to-end is a
    // cross-component client-contract change left for a follow-up; it also
    // affects the existing file_change card.)
    flushPendingAsActivity(ctx, state);
    publishDurable(ctx, state, 'publishText', {
