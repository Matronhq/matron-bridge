...
export async function validateAndOpen(filePath, { workdir, allowedRoots, maxBytes = MAX_VIEW_BYTES, strictSnapshot = false } = {}) {
  ...
  try {
    ...
    // A size change between the initial stat and read completion means the
    // file grew/shrank while we read. Strict callers (show_file) reject the
    // torn snapshot; serve callers (/view, /download) return the bounded
    // snapshot of exactly the bytes read.
    if (strictSnapshot && finalStat.size !== stat.size) throw new FileLinkDenied('unreadable');
