...
// webBaseUrl is where the web app itself is served (it owns the `#files=` hash router).
// UNLIKE generateFileLink (index.js), which mints a signed, TTL'd viewer token, this carries NO
// token: the target is opened through the user's EXISTING web session (the same auth every
...
// Append a user-visible "open in Files" line to an attachment caption or item body. Additive:
