...
export function createQueuedReleaseOutbox({ file = DEFAULT_FILE, log = console } = {}) {
  ...
  // crashed mid-write (never an in-flight save — construction precedes the
  // first saveAll). Quarantined `.corrupt-*` files are swept only past
  ...
  return {
    ...
    markAcked(promptId, action) {
      ...
      if (count > 0) {
        ...
        // sweepAcked saves when it drops something; otherwise persist the flip.
