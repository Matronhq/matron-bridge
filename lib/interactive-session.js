...
import { PromptDetector, stripAnsi } from './prompt-detector.js';
...
export class InteractiveSession extends EventEmitter {
  ...
  // Return the current visible PTY screen as a plain (ANSI-stripped) string.
  // Used by the bridge to surface free-text TUI output (e.g. the /login
  // OAuth URL screen) that doesn't fire transcript events and isn't
  // detectable as a numbered/lettered/yes-no menu.
  getCurrentScreenText() {
    return stripAnsi(this.detector.buf || '');
  }
}
