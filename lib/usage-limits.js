...
export function parseUsageLimits(rawText, now = new Date()) {
  ...
  for (const line of String(rawText ?? '').split('\n')) {
    ...
    parsed.push({
      ...
      // the resets/resets_at/resets_at_ms keys entirely (never emit null).
  ...
  for (const p of parsed) {
    ...
    // resets_at stays an ISO string (unchanged wire type); resets_at_ms adds the
    // epoch number. All three (resets, resets_at, resets_at_ms) are present only
    // when a resets clause exists AND parses; otherwise all omitted.
