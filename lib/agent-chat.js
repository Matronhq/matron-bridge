  ...
  async function answerInvite(data, accept) {
    ...
    if (room.role === 'owner') {
      ...
      if (admit.kind !== 'answered') {
        ...
        // Journal rejected the admit: the request it referenced is dead
        // server-side, so consume it — the requester can re-ask.
        clearPendingPeer(chatRoomId);
        return {
          status: admit.code === 'conflict' ? 409 : 502,
          body: { error: `journal rejected the admit: ${admit.detail || admit.code || 'unknown error'}` },
        };
      }
      clearPendingPeer(chatRoomId);
    ...
    if (outcome.kind !== 'answered') {
      ...
      // pending -> expired is an allowed transition; the invite is dead
      // server-side and this binding must never turn routable.
      rooms.setState(chatRoomId, 'expired');
      return {
        status: outcome.code === 'conflict' ? 409 : 502,
        body: { error: `journal rejected the accept: ${outcome.detail || outcome.code || 'unknown error'} — the invite is gone; ask for a fresh one` },
      };
    }
