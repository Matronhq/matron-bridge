...
export function createSendAttachmentHandler({ sessions, publisher, journalConvoIdFor, rooms = null, maxBytes = MAX_ATTACHMENT_BYTES }) {
  return async function handleSendAttachment(data) {
    ...
    // convo instead of the session's own — same joined-or-owner gate as
    // agent_chat_send (lib/agent-chat.js boundRoom).
    let convoId;
    if (chatRoomId !== undefined) {
      ...
      if (!room || room.sessionRoomId !== roomId) return { status: 404, body: { error: `not a participant of room ${chatRoomId}` } };
      if (room.state !== 'joined' && room.role !== 'owner') {
