...
// tokens (see compactScreenText); the matched token doubles as the cue's
// canonical name in the screen-update dedup signature, so "Paste code here"
// and "Press Enter to continue" stay distinguishable ('pastecode' vs
// 'pressenter').
const INPUT_CUE_TOKENS = [
  'pastecode',
  'pressenter',
  'entertocontinue',
  'copytheurl',
...
// Returns the canonical cue token present on the screen, or '' when none is.
export function findInputCue(screen) {
  const compact = compactScreenText(screen);
  return INPUT_CUE_TOKENS.find(t => compact.includes(t)) || '';
}
...
const READY_IDLE_TOKENS = [
  ...
  'forshortcuts',
  'bypasspermissionson',
  // Logged-out TUI ("Not logged in · Please run /login" / "· Run /login"):
  // idle in every way that matters — it accepts typed slash commands. A
  // resumed-while-logged-out session never shows the normal idle footer, so
  // without this token a parked /login only ran at the 120s hardcap while
  // the user's retries all bounced off "the session is still resuming".
  'notloggedin',
];
