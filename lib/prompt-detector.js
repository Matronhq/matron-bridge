...
export class PromptDetector extends EventEmitter {
  ...
  _check() {
    ...
    // No structured prompt — check for free-text content the user must see.
    // We only emit when the screen has URLs (always actionable) OR an input
    // cue ("paste code here", "press enter to continue") so we don't spam
    // Matrix on every idle status redraw.
    const urls = extractUrls(screen);
    const hasCue = INPUT_CUE_RE.test(screen);
    if (urls.length === 0 && !hasCue) return;
    // Dedupe on the sorted URL set + cue presence — re-renders with the same
    // URL/cue produce the same sig and don't re-fire.
    const sig = `${urls.slice().sort().join(',')}::cue=${hasCue}`;
    if (sig === this.lastScreenUpdateSig) return;
    this.lastScreenUpdateSig = sig;
    this.emit('screen-update', { screen, urls, hasInputCue: hasCue });
  }
