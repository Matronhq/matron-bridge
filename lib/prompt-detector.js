...
export function classifyScreen(screen) {
  ...
  if (YN_RE.test(screen)) {
    ...
    return {
      ...
      freeTextIdx: null, // yes/no prompts have no free-text slot
    };
  }
  ...
  // preferring runs closer to the bottom of the screen.
  {
    ...
    for (const run of runs.reverse()) {
      ...
      if (Number.isFinite(firstNum) && firstNum === 2 && questionLines.length > 0) {
        ...
        // heading text itself can legitimately contain "1." earlier (e.g.
        // "Step 1. Pick a theme  1. Auto (match terminal)") and matching
        // the leftmost would mis-extract the heading as the option label.
