...
export function classifyScreen(screen) {
  ...
  // PLUS evidence that this is an interactive menu rather than just a
  // numbered list inside prose: either a question/instruction line above
  // (ends with `?` or `:`), or a selection marker (❯) on the first item.
  {
    const items = collectConsecutive(lines, NUMBERED_LINE_RE);
    if (items.length >= 2) {
      const opts = items.matches.map(m => ({ key: m[1], label: m[2].trim() }));
      const above = lines.slice(Math.max(0, items.startIdx - 2), items.startIdx).map(l => l.trim()).filter(Boolean);
      const question = above.join(' ').trim();
      const aboveLooksInterrogative = /[?:]\s*$/.test(question);
      const firstItemMarked = /^[\s]*[❯>▶►]/.test(lines[items.startIdx] || '');
  ...
  // Lettered selection — same guard as numbered.
  {
