...
import { AGENT_CLAUDE, normalizeAgent } from './agent-backend.js';
...
export function createRpcRequestHandler({
  ...
  const modelSelectable = normalizeAgent(defaultAgent) === AGENT_CLAUDE;
  ...
  const handlers = {
    ...
    start(request) {
      ...
      // on a box that starts Codex sessions, "opus" is not an unknown Claude
      // alias, it is a request this box cannot honour at all.
      if (hasModel && !modelSelectable) {
        return respond(request, false, {
          code: 'bad_model',
          detail: 'this box starts Codex sessions; Claude model aliases do not apply',
        });
      }
      ...
      let session;
      try {
        session = startSession({ workdir, mcpExtras, ...(model ? { model } : {}) });
      } catch (e) {
