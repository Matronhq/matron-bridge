...
export function createRpcRequestHandler({
  ...
  serverLabel = '',
  // Guarded file RPCs (read_file / edit_file). getFileAllowedRoots is a thunk
  // returning the PINNED allowed-root set (pinAllowedRootsSync output),
  // resolved once at the trusted boundary in index.js and never rebuilt from
  // client strings. Null/empty -> both fail closed with bad_workdir.
  getFileAllowedRoots = () => null,
  // read_file and edit_file are registered only when this is true; otherwise
  // both are absent and a caller gets unknown_method, as from an older bridge.
  ...
  const handlers = {
    ...
    // Guarded file edit: apply a full-content or a targeted old->new-string
    // edit to an EXISTING file inside the bridge's pinned allowed roots. All
    // path-safety + the atomic write live in lib/edit-file.js (which composes
    // file-link-guard + atomic-write); this handler only validates the RPC
    // envelope, hands the pinned roots to the edit, and maps
    // EditFileError.code onto the wire error body. Async (validateAndOpen
    // is); the dispatch below turns a rejection into the one reply owed.
    async edit_file(request) {
      ...
      try { allowedRoots = getFileAllowedRoots(); } catch { /* fail closed below */ }
      try {
    ...
    // Guarded file read: return an EXISTING file's
    ...
    // ReadFileError.code onto the wire error body.
  ...
  if (!fileEditEnabled) {
    delete handlers.edit_file;
    delete handlers.read_file;
  }
  ...
  return function handleRpcRequest(request) {
    try {
      ...
      if (out && typeof out.then === 'function') {
        // Returned so a caller (and a test) can await the reply.
        return out.catch((e) => {
