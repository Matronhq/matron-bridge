...
import { oneLine } from './peer-text.js';
...
export function composeSpawnOpeningTurn({ task, roomId, fromName, serverLabel }) {
  const flatFromName = oneLine(fromName);
  const flatServerLabel = oneLine(serverLabel);
  const parent = flatFromName ? `the user's agent session on "${flatFromName}"` : `another of the user's agent sessions`;
  const here = flatServerLabel ? ` You are running on "${flatServerLabel}".` : '';
  return [
