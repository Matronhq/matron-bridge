...
//
// Read counterpart of lib/edit-file.js. An in-client editor
...
export async function readFileGuarded(input, {
  ...
  // Stricter than the shared guard: this reply carries the bytes off the box.
  if (isCredentialPath(realPath)) throw new ReadFileError('sensitive', 'path rejected: sensitive');

  // Refuse a file whose bytes don't SURVIVE a utf8 round-trip. The editor
  ...
  // Authoritative frame-cap guard: the raw-read cap bounds bytes, but JSON
