...
// The session's default tool allow-list. The classifier consumes only the
// mcp__* entries; the non-mcp entries are the session's baseline capabilities,
// listed so the snapshot's default layer matches what the bridge grants. (In our
// fork this comes from a session-settings builder that also carries PreToolUse
// hook wiring — that belongs with the permission transport, not the classifier,
// so it is inlined here to keep this PR to the classifier.)
const DEFAULT_SESSION_PERMISSIONS = {
  allow: [
    'Bash(*)', 'Read(*)', 'Write(*)', 'Edit(*)', 'MultiEdit(*)', 'Glob(*)', 'Grep(*)',
    'WebFetch(*)', 'WebSearch(*)', 'Skill', 'Agent(*)', 'Task(*)', 'NotebookEdit(*)',
    'mcp__show-file__show_file',
  ],
  deny: [],
};
...
export function buildPermissionSnapshot({ workdir = process.cwd(), sourcePaths } = {}) {
  ...
  addPermissions(mutable, DEFAULT_SESSION_PERMISSIONS);
