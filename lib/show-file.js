...
export async function shareAgentMedia({
  ...
  // Files deep link: open the shown file in the web client's Files pane.
  // realPath already cleared validateAndOpen's fd-pinned containment; the link
  // is gated again on the pinned root that holds it (re-running the sensitive
  // name check). Null (webBaseUrl unset, or the gate denies) leaves the
  // caption exactly as the agent passed it.
  const rootDir = pinnedRoots.roots
    .map((r) => r.realPath)
    .find((r) => realPath === r || realPath.startsWith(r.endsWith(path.sep) ? r : r + path.sep)) || null;
  const deepLink = rootDir ? buildFilesDeepLink(realPath, rootDir, deps.webBaseUrl) : null;
