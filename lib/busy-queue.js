...
export async function notifyQueuedMessage(session, preview, {
  ...
  const question = allowMakeTask
    ? `${sendCancelQuestion} Or file it as a task instead.`
    : sendCancelQuestion;
  const actions = [
...
export function resolveQueueReleaseTap(value, session, {
  ...
  if (entry) {
    ...
    if (value === 'make_task') {
      ...
      if (!tappedId || !Array.isArray(queue) || !Array.isArray(notifs)) return true;
      const index = notifs.findIndex(notification => notification?.id === tappedId);
      // Stale tile — the message already flushed or was cancelled from another
      // client. Same silence as every other queue action.
      if (index < 0 || index >= queue.length) return true;
      ...
      if (!isTextOnlyQueueEntry(blocks)) return true;
      const text = blocks.map(block => block.text).join('\n');
      // A notice must never break the tap, and this runs inside a promise
      // chain whose rejection nobody is listening to.
      const say = (message) => {
      ...
      Promise.resolve()
        ...
        .then((result) => {
          ...
          // whether the queued copy is still going to be sent as a message.
          say(released
            ? `📌 Filed as task #${result.num}. The agent will be told when this turn ends.`
            : `📌 Filed as task #${result.num}, but the queued message couldn't be withdrawn — it will still be sent.`);
        })
        .catch((e) => {
