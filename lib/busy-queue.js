...
export async function notifyQueuedMessage(session, preview, {
  ...
  // and question are worded to be honest about that asymmetry.
  const queuedReleasePayload = {
    kind: 'queued_release',
    prompt_id: promptId,
    question: 'Send all queued messages now, or cancel this one?',
    items: [{ id: itemId, text: fullText }],
    actions: [
      { id: 'send', label: 'Send all now', intent: 'primary' },
      { id: 'cancel', label: 'Cancel this', intent: 'neutral' },
    ],
    body: plainNotif,
  };
