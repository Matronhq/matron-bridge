...
export async function notifyQueuedMessage(session, preview, {
  ...
  // requests, and therefore the only one that offers both. Until send_one
...
export function resolveQueueReleaseTap(value, session, {
  ...
  if (entry) {
    ...
    if (value === 'send_one') {
      ...
      } else if (notification && queueBatchWasRestored(session, blocks)) {
        restoreQueueNotifications(session, [notification]);
      }
      return true;
    }
