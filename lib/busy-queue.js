...
// Is a queued entry made only of text the user wrote? The single definition of
// what "📌 Make task" can file: index.js asks it when deciding whether a card
// may OFFER the action, and the tap branch below asks it again before filing,
// because a durable card outlives the queue state that produced it. A media
// entry (saved file, image, extracted video frames) has no sentence to put in
// a task; an empty entry has nothing at all.
export function isTextOnlyQueueEntry(blocks) {
  return Array.isArray(blocks) && blocks.length > 0
    && blocks.every(block => block && block.type === 'text' && typeof block.text === 'string');
}
...
export async function notifyQueuedMessage(session, preview, {
  ...
  allowSendOne = false,
  // May this card offer "📌 Make task" (items tracker, spec 2026-09-08)? Also
  // opt-in, for the same reason as allowSendOne but a different constraint:
  // the action files the queued MESSAGE TEXT as a tracker task, so it only
  // means anything for a text entry whose text the user wrote. A queued
  // image, saved file or extracted video has no sentence to file, and the
  // bridge's own synthetic turns (an item marker rendered for the agent) are
  // not the user's words at all. Call sites decide; the tap branch below
  // re-checks the entry shape rather than trusting the flag.
  allowMakeTask = false,
} = {}) {
  ...
  const sendCancelQuestion = jumped > 0
    ...
    : single
      ...
      : multiQuestion;
  // Appended rather than folded into the three shapes above: every question
  // already varies on single/multi/compact, and spelling make-task into each
  // would square that matrix for one extra sentence. The existing strings are
  // untouched when the action isn't offered, which is every pre-tracker card.
  const question = allowMakeTask
    ? `${sendCancelQuestion} You can also file this message as a task instead.`
    : sendCancelQuestion;
  const actions = [
    { id: 'send', label: sendLabel, intent: 'primary' },
    ...(multi ? [{ id: 'send_one', label: '⚡ Send just this one', intent: 'neutral' }] : []),
    ...(allowMakeTask ? [{ id: 'make_task', label: '📌 Make task', intent: 'neutral' }] : []),
...
export function resolveQueueReleaseTap(value, session, {
  ...
  // async (session, { text }) -> { ok: true, num, id } | { ok: false, error }.
  // Files the tapped message in the tracker (index.js owns the journal call
  // and the agent's heads-up). Absent for callers that predate the tracker.
  makeTask = null,
} = {}) {
  ...
  if (entry) {
    ...
    // sending it (items tracker, spec 2026-09-08). Async by necessity (a
    // journal write), so the tap is acknowledged synchronously and the outcome
    // lands as a notice.
    //
    // Deliberately NOT detach-then-write, which is what send_one does and what
    // this branch was first specified as. Two reasons, both structural:
    //   * restoreDetachedQueueItem only puts an entry back when flushQueue
    //     handed it back at the front of the queue (queueBatchWasRestored).
    //     Nothing hands it back here, so a failed journal write would restore
    //     NOTHING and the user's message would be gone.
    //   * cancelQueuedItem resolves the item by id and does the durable
    //     write-ahead BEFORE its splice — the ordering that makes a cancel
    //     survive a crash. A prior detach both defeats that ordering and hides
    //     the item from its own id lookup.
    // So: write first, then release. The window between the two is benign —
    // the message is simply still queued, and a turn-end flush that beats us
    // to it sends it (the task is filed either way, and the id lookup then
    // finds nothing to release, which is exactly right).
    if (value === 'make_task') {
      // No seam wired: this caller cannot honour the action. Report it
      // unhandled rather than swallowing the tap silently.
      if (typeof makeTask !== 'function') return false;
      // A notice must never break the tap, and the outcome half of this runs
      // inside a promise chain whose rejection nobody is listening to.
      const say = (message) => {
        try { if (typeof notify === 'function') notify(message); }
        catch { /* the outcome is already decided; telling is best effort */ }
      };
      const tappedId = Array.isArray(entry.itemIds) ? entry.itemIds[0] : null;
      const queue = session.queuedMessages;
      const notifs = session.queueNotifications;
      const index = tappedId && Array.isArray(queue) && Array.isArray(notifs)
        ? notifs.findIndex(notification => notification?.id === tappedId)
        : -1;
      // Stale tile — the message already flushed, or was cancelled from
      // another client. Unlike send/cancel, whose stale taps are silent
      // because the user's intent (get rid of it / send it) has happened
      // anyway, a silent make_task looks exactly like a successful one: no
      // task, no message, no word about either.
      if (index < 0 || index >= queue.length) {
        say('That queued message is no longer here — nothing was filed.');
        return true;
      }
      const blocks = queue[index];
      // Media entries are never offered the action (allowMakeTask), but the
      // card is durable and the queue is shared: re-check rather than trust.
      if (!isTextOnlyQueueEntry(blocks)) {
        say("That queued message isn't text, so there's nothing to put in a task.");
        return true;
      }
      const text = blocks.map(block => block.text).join('\n');
      // One filing per queued message at a time. The tap is acknowledged
      // synchronously and the card stays on screen until the journal write
      // resolves, so an impatient second tap during a slow POST would file a
      // DUPLICATE task — and its release would then find nothing to remove and
      // tell the user, falsely, that the message is still going out. Keyed by
      // the tapped item id (not the card) so two different queued messages can
      // still be filed concurrently.
      const inFlight = session._makeTaskInFlight instanceof Set
        ? session._makeTaskInFlight
        : (session._makeTaskInFlight = new Set());
      if (inFlight.has(tappedId)) return true;
      inFlight.add(tappedId);
      Promise.resolve()
        .then(() => makeTask(session, { text }))
        .then((result) => {
          if (!result || !result.ok) {
            say(`Couldn't file that as a task (${result?.error || 'unknown error'}). It is still queued.`);
            return;
          }
          let released = false;
          try {
            released = cancelQueuedItem(session, {
              itemId: tappedId,
          ...
          // The item is filed either way — never say it wasn't. What changes is
          // whether the queued copy still goes to the agent as a message, and
          // the two ways that happens (a turn-end flush beat us to it; the
          // durable release fail-closed) are indistinguishable from here, so
          // the wording covers both rather than guessing.
          say(released
            ? `📌 Filed as task #${result.num}. The agent will be told when this turn ends.`
            : `📌 Filed as task #${result.num}, but the queued message was already sent or could not be withdrawn.`);
        })
        .catch((e) => {
          say(`Couldn't file that as a task (${e?.message || e}). It is still queued.`);
        })
        .finally(() => { inFlight.delete(tappedId); });
      return true;
    }
