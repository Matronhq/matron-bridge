...
export function createAgentInvites({ sendRoomOp, rooms, injectRequestTurn, notifyRoom, log = console } = {}) {
  ...
  // Sticky per-consumer capability flag: set on the first room_id-stamped op
  // error seen from this journal. A journal that stamps once stamps every
  // correlatable error, so once true an UNSTAMPED error must settle NOTHING —
  // the ref-level scan in onOpError would settle whichever room armed first,
  // which can be a different room than the one that failed (a false terminal
  // 'left'/'answered', unrecoverable without a restart). A never-stamping
  // journal never sets it and keeps the scan behavior unchanged.
  let journalStampsRoomIds = false;
  ...
  return {
    ...
    onOpError({ code, ref, detail, roomId = null }) {
      ...
      // error.
      if (roomId) {
        journalStampsRoomIds = true;
        settleReturns(roomId, `error:${ref}`, { kind: 'error', code, detail });
        return;
      }
      // Once this journal has proven it stamps, an unstamped error is
      // uncorrelatable and must not scan: see journalStampsRoomIds above.
      if (journalStampsRoomIds) return;
      // Old-journal fallback (no room_id on error frames): correlate at
      // ref level. Room ops are serialized per tool call, so that
      // granularity is enough. Settle at most ONE waiter per error frame:
      // op errors are 1:1 with ops and the socket preserves order, and
      // eviction fires a BATCH of agent_leave ops — one owner-room
      // conflict must not fail every in-flight leave (scoped re-review of
      // the C2 fix).
      for (const [rid] of waiters) {
        if (settleReturns(rid, `error:${ref}`, { kind: 'error', code, detail })) return;
      }
    },
  };
}
