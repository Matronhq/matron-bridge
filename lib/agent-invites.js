...
export function createAgentInvites({ sendRoomOp, rooms, injectRequestTurn, notifyRoom, log = console } = {}) {
  ...
  return {
    ...
    onOpError({ code, ref, detail }) {
      ...
      // Settle at most ONE waiter per error frame: op errors are 1:1 with
      // ops and the socket preserves order, and eviction fires a BATCH of
      // agent_leave ops — one owner-room conflict must not fail every
      // in-flight leave (scoped re-review of the C2 fix).
      for (const [roomId] of waiters) {
        if (settleReturns(roomId, `error:${ref}`, { kind: 'error', code, detail })) return;
      }
    },
  };
}
