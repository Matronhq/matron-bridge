...
export function formatInviteRequestNotice(frame, { roomTitle = null } = {}) {
  ...
  return `🤝 ${who} requests a chat${topic ? ` about "${topic}"` : ''}${because}`;
}
...
export function createAgentInvites({ sendRoomOp, rooms, injectRequestTurn, notifyRoom, log = console } = {}) {
  ...
  return {
    ...
    async invite({ roomId, targetDeviceId, targetConvoId, topic, justification }) {
      ...
      if (!sendRoomOp({ op: 'agent_invite', room_id: roomId, target_device_id: targetDeviceId, ...(targetConvoId ? { target_convo_id: targetConvoId } : {}), ...(topic ? { topic } : {}), justification })) {
