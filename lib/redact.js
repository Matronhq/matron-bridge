...
export function createPublishRedactor({
  ...
  return value => {
    ...
    // by size. Under the bound (or with no operator patterns) apply them normally.
    if (!patterns.length || output.length <= maxOperatorPatternBytes) {
    ...
    if (!oversizeWarned) {
      ...
      try {
        log?.warn?.(
          `[publish-redactor] input ${output.length}B exceeds ` +
    ...
    if (resolvedOversizePolicy === 'drop') {
      return `[REDACTED-OVERSIZE: ${output.length}B payload withheld ` +
    ...
    // no operator-pattern-eligible bytes publish unredacted. Cut on the last line
    // boundary inside the bound so a secret straddling the cut isn't split into a
    // publishable prefix; fall back to a hard slice for one enormous line.
    let head = output.slice(0, maxOperatorPatternBytes);
    const lastNewline = head.lastIndexOf('\n');
    if (lastNewline >= maxOperatorPatternBytes >> 1) head = head.slice(0, lastNewline);
    const droppedBytes = output.length - head.length;
    return `${applyOperatorPatterns(head)}\n` +
      `[REDACTED-OVERSIZE: ${droppedBytes}B dropped beyond the ` +
