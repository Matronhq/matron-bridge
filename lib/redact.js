...
export function createPublishRedactor({
  ...
  return value => {
    ...
    // Bound the operator-supplied patterns to defend the event loop against an
    // accidental catastrophic-backtracking rule on a large input. The baseline
    // above always runs; only the extra value-format rules are skipped over the
    // threshold, and we warn once so the coverage gap is visible in the logs.
      ...
      if (!oversizeWarned) {
        ...
        try {
          log?.warn?.(
            ...
            `${maxOperatorPatternBytes}B; skipping ${patterns.length} operator ` +
            'pattern(s) for this and larger inputs (baseline redaction still applied)',
          );
