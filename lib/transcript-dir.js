...
// How much of a transcript's tail to scan for a `cwd` field. Every entry
// carries one, so any chunk holding at least one complete line is enough;
// 256KB comfortably covers even a single huge tool-result line.
...
// the session was actually running. Reads a bounded tail chunk and scans
// lines newest-first; the chunk's first line may be truncated mid-JSON, which
// the per-line try/catch skips.
function lastRecordedCwd(transcriptPath) {
  let fd;
  try {
    fd = fs.openSync(transcriptPath, 'r');
    const size = fs.fstatSync(fd).size;
    const len = Math.min(size, CWD_SCAN_BYTES);
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        // truncated or malformed line — keep scanning older lines
      }
    }
    return null;
  } catch {
