  ...
  function describeOutcome(frame, ctx) {
    ...
    switch (frame.outcome) {
      case 'started': {
        ...
        // The plain "Child conversation: <id>." sentence below keeps this
        // sanitized-but-unencoded id, unchanged for anything parsing it;
        // only the markdown link target needs the extra linkSafeId encoding
        // (see its own comment above).
        const linkHint = `Link it for the user as [title](matron://convo/${linkSafeId(childConvoId)}).`;
