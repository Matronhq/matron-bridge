  ...
  function describeOutcome(frame, ctx) {
    ...
    switch (frame.outcome) {
      case 'started': {
        ...
        // childConvoId is target-bridge-authored (see safeFrameField's own
        // note above) and only flattened to one line — a raw `)` in it
        // would close this markdown link early and splice arbitrary text
        // into a notice the bridge signs and publishes to the user's own
        // chat. encodeURIComponent alone is NOT enough: '(' and ')' are in
        // its unreserved set (a JS/ECMA-262 quirk — they're legal URI marks
        // under RFC 2396) and pass through raw, which is exactly the
        // character this is meant to neutralise, so they get an explicit
        // extra pass. Applied only to the link target; the plain "Child
        // conversation: <id>." sentence above keeps the sanitized-but-
        // unencoded id, unchanged for anything parsing it.
        const linkSafeConvoId = encodeURIComponent(childConvoId).replace(/[()]/g, (c) => (c === '(' ? '%28' : '%29'));
        const linkHint = `Link it for the user as [title](matron://convo/${linkSafeConvoId}).`;
        return frame.room_id
          ? `🚀 Spawn ${id}: ${prefix}session started on the target box. Chat room ${safeFrameField(frame.room_id)} is the channel; the child was seeded with your task and will report there. Child conversation: ${childConvoId}. ${linkHint}`
          : `🚀 Spawn ${id}: ${prefix}session started on the target box, detached — it was seeded with your task and will not report back. Child conversation: ${childConvoId}. ${linkHint} If you later need a channel to it, agent_chat_start can open one.`;
      }
      case 'declined':
