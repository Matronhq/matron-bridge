...
// notice when a tool call crosses the threshold, one reminder if it is STILL
// running at 5x the threshold, and nothing at all for the overwhelming
// majority of calls that finish quickly.
//
// Scope decisions:
...
//   design. EVERY other tool is covered — built-in, MCP and subagent
//   (Task/Agent) alike: a subagent that runs long on purpose costs two
//   lines an hour, a wedged one is otherwise indistinguishable from a
//   dead session, and the 2026-09-05 chrome-devtools hang (two
//   get_network_request calls, 30 min each, message queued in silence)
//   showed the cost of leaving any tool out.
...
export const DEFAULT_SLOW_TOOL_NOTICE_MS = 60_000;
export const MAX_SLOW_TOOL_NOTICE_MS = 3_600_000;
export const REMINDER_MULTIPLIER = 5;

// Env knob (MATRON_SLOW_TOOL_NOTICE_MS): unset/invalid falls back to the
// default; 0 or negative disables the feature; out-of-range high values
// clamp to the default rather than arming an hour-plus timer nobody meant.
export function resolveSlowToolNoticeMs(raw) {
  if (raw === undefined || raw === null || raw === '') return DEFAULT_SLOW_TOOL_NOTICE_MS;
  const ms = Number(raw);
  if (!Number.isFinite(ms)) return DEFAULT_SLOW_TOOL_NOTICE_MS;
  if (ms <= 0) return 0;
  return ms <= MAX_SLOW_TOOL_NOTICE_MS ? ms : DEFAULT_SLOW_TOOL_NOTICE_MS;
}

const EXCLUDED_TOOLS = new Set(['AskUserQuestion', 'ExitPlanMode', 'EnterPlanMode']);
...
export function createSlowToolNotices({
  ...
  return {
    toolStarted(id, toolName) {
      ...
      const timers = [
        setTimer(() => fire(id, false), thresholdMs),
        setTimer(() => fire(id, true), thresholdMs * REMINDER_MULTIPLIER),
      ];
