...
function bindingIsLive(state, updatedAt) {
  if (state === 'joined') return true;
  return state === 'pending' && Date.now() - (updatedAt || 0) < INVITE_TTL_MS;
}
  ...
  return {
    ...
    findLivePair(sessionRoomId, { peerDeviceId = null, targetConvoId = null, peerSessionRoomId = null } = {}) {
      ...
      for (const [id, r] of Object.entries(rooms)) {
        ...
        if (!bindingIsLive(isPrimary ? r.state : r.guestState, r.updatedAt)) continue;
        if (r.guestSessionRoomId != null) {
          ...
          if (!bindingIsLive(otherState, r.updatedAt)) continue;
        } else {
