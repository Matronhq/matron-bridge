...
// A wrapper-producer (son-of-anton's codex_execute.sh, signalled by
// MATRON_CODEX_REAL_BIN) counts ONLY when the path is set AND actually resolves.
// A set-but-missing REAL_BIN is not a producer — treating it as one would both
// enable detectProducer's watcher with nothing behind it AND suppress the shim
// PATH-prepend in configureCodexSinkEnv, leaving the session with no producer at
// all (the silent empty-view this guard exists to prevent). Both call sites MUST
// share this predicate so they never disagree on whether a wrapper is deployed.
function hasResolvedRealBin(env) {
  const realBin = typeof env.MATRON_CODEX_REAL_BIN === 'string' ? env.MATRON_CODEX_REAL_BIN.trim() : '';
  return Boolean(realBin && safeRealpath(realBin));
}
...
export function configureCodexSinkEnv({
  ...
  try {
    if (env.MATRON_CODEX_VIZ === '1') {
      ...
      // wrapper is the sole producer, and its own internal bare
      ...
      // never both. Use the SAME validated predicate as detectProducer so a
      // set-but-missing REAL_BIN doesn't suppress the shim here while
      // detectProducer reports no producer — that mismatch is the empty view.
      if (!hasResolvedRealBin(env)) {
