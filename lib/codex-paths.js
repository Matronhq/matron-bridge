...
// The distinct "an external wrapper is the producer" signal. A son-of-anton-style
// integration whose own launcher (codex_execute.sh) is a redaction-aware producer
// sets MATRON_CODEX_PRODUCER=wrapper to declare it. This is DELIBERATELY separate
...
//   1. MATRON_CODEX_PRODUCER=wrapper — an external wrapper (the son-of-anton
//      integration point, codex_execute.sh) has declared itself the producer.
...
// real codex behind it forwards to nothing. MATRON_CODEX_REAL_BIN counts when it
// resolves: a resolvable REAL_BIN is a runnable codex regardless of PATH. This is
// a DIFFERENT question from the producer guard above (detectProducer keys on the
// MATRON_CODEX_PRODUCER=wrapper signal, not REAL_BIN), so the REAL_BIN-resolves
// check lives here alone and is not shared with the guard. Feeds the New Chat
