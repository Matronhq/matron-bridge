...
// Removes per-session sink dirs whose most-recent mtime is older than the
// retention window. Never throws (viz + its cleanup are both optional).
export function pruneStaleCodexSinks({
  now = Date.now,
  maxAgeMs = Number(process.env.MATRON_CODEX_SINK_RETENTION_MS) || DEFAULT_SINK_RETENTION_MS,
  root = codexSinksRoot(),
  fsImpl = fs,
  warn = console.warn,
} = {}) {
