...
//
// `effort` is the ONE deliberate exception to that rule, because it is the
// only field that can transition BACK to unknown (session start, restart and
// resume all reset it — see lib/effort-tracker.js). Clients merge status
// fields stickily, so under the omit rule an absent effort reads as
// "unchanged" and the app would go on rendering the pre-restart level: the
// precise false confidence the reset exists to prevent. Omission cannot
// express "forget what you had", so effort is tri-state on the wire:
//
//   absent        -> no opinion; keep whatever you last rendered
//                    (Codex sessions, subagent frames)
//   null          -> UNKNOWN; clear whatever you last rendered
//   "<level>"     -> the tracked level
//
// The null is re-published on EVERY frame while unknown rather than sent once
// as a clear, so a client that missed a frame still converges.
//
...
// lib/effort-command.js). They are SESSION-SCOPED, not global: the caller
// passes null for an agent that offers none (Codex takes a free-text model id
// the bridge never validates, and doesn't expose effort at all), and the
// field is omitted rather than shipped empty so clients keep what they last
// rendered.
//
// `effort` is the current level, optimistically tracked by
// lib/effort-tracker.js — the bridge cannot read it back. Unknown is an
...
export function buildSessionStatus({ model, contextTokens, limits, email, vitals, workdir, modelOptions, effortLevels, effort } = {}) {
  ...
  if (Array.isArray(modelOptions) && modelOptions.length > 0) status.model_options = modelOptions;
  if (Array.isArray(effortLevels) && effortLevels.length > 0) status.effort_levels = effortLevels;
  // Tri-state (see the exception documented above): `undefined` omits the
  // field, anything else publishes — a non-empty string as itself, unknown as
  // an explicit null.
