...
// stamped alongside _cachedCpuPct on every SUCCESSFUL sample so hostVitalLimits()
...
// as if it were current (#526 stale-replay blocker).
...
export function sampleCpuOnce() {
  ...
  _cachedCpuSampledAtMs = Date.now(); // age of THIS valid reading (#526)
}
...
// is available.
//
// Each entry carries sampled_at_ms (epoch ms) so clients can expire/hide a stale
// reading: the publisher replays the last status frame to new viewers, so an idle
// convo would otherwise render an arbitrarily old host reading as current (#526).
// host_cpu uses the sampler's cache-stamp (its 15s cadence); host_ram is read
// inline here, so its sample time is now.
