...
// model a new chat starts on when nobody picked one. A resumed room keeps
// its persisted model regardless: the fallback only decides FRESH starts.
//
...
export function resolveModel({ option, persisted, fallback }) {
  const picked = option ?? persisted;
  if (picked === 'default' && fallback) return fallback;
  return picked ?? fallback ?? undefined;
}
