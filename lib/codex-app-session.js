...
import { normalizeCodexSandbox } from './codex-session.js';
...
export function codexPlanConfig(config = {}, defaults = {}) {
  ...
  return { ...config,
    ...
    browser_use: { ...browser, default_origin_policy: { access: 'deny', uploads: 'deny', downloads: 'deny', full_cdp_access: 'deny' },
      origins: Object.fromEntries(Object.entries(browser.origins || {}).map(([origin, policy]) => [origin, { ...policy, access: 'deny' }])) },
...
export class CodexAppServerSession extends EventEmitter {
  constructor({ cwd, threadId = null, model = null, effort = null, sandbox = 'workspace-write',
  ...
  notification({ method, params = {} }) {
    ...
    if (params.turnId && this.finishedTurns.has(params.turnId)) return;
