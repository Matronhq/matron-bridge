...
export async function withCodexAppServer(callback, {
  ...
  // Account / model / rate-limit queries never touch the journal: spawn the
  // app-server without the journal credential or bridge-only secrets. Provider
  // keys stay: Codex can authenticate with OPENAI_API_KEY, and the account it
  // reports must match the one sessions run as. The helper copies, so the
  // caller's env (default process.env) is untouched.
  const child = spawnImpl(command, ['app-server', '--listen', 'stdio://'], {
    cwd, env: stripJournalCreds(env, { keepProviderKeys: true }), stdio: ['pipe', 'pipe', 'pipe'],
  });
