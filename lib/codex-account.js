...
export async function withCodexAppServer(callback, {
  ...
  // app-server without the journal credential or bridge-only secrets. The
  // helper copies, so the caller's env (default process.env) is untouched.
  const child = spawnImpl(command, ['app-server', '--listen', 'stdio://'], {
    cwd, env: stripJournalCreds(env), stdio: ['pipe', 'pipe', 'pipe'],
  });
