...
export function createItemsHandlers({ sessions, journalConvoIdFor, client, uploadLocalFile, webBaseUrl = '' }) {
  ...
  async function uploadAll(session, paths) {
    ...
    for (const p of paths) {
      ...
      // Files deep link: so the user can browse to the handed-off doc rather than
  ...
  // so they surface in the user's item thread alongside the inline attachment. Additive: the
  ...
  return {
    async create(data) {
      ...
      if (finalBody !== undefined) body.body = finalBody;
