...
export function createRoomDelivery({ isBusy, injectTurn, log = console } = {}) {
  ...
  // can forge a `[room "..."]` header or another sender's line. The flattener
  // itself lives in lib/peer-text.js so the user-facing notices (which have
  // the same forgery problem, in Dan's chat rather than the agent's turn)
