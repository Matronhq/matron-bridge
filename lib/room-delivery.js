  ...
  return {
    ...
    carryForward(fromKey, toKey) {
      ...
      if (list) {
        ...
        // Append to (never clobber) anything already pending at the new key.
        pending.set(toKey, [...(pending.get(toKey) || []), ...list]);
      }
      const d = dropped.get(fromKey);
      if (d) {
        dropped.delete(fromKey);
        const merged = dropped.get(toKey) || new Map();
        for (const [roomId, n] of d) merged.set(roomId, (merged.get(roomId) || 0) + n);
        dropped.set(toKey, merged);
      }
    },
  };
}
