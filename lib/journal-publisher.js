...
export function createJournalPublisher({
  ...
  // into a notice slot by a mis-wired caller) would render as raw
  // JSON in every client, so the sink refuses it loudly instead.
  // Fail-open like every other publish entry point: a hostile payload (a
  // throwing getter) is refused, never thrown into the caller.
  function hasStringTextBody(payload, method) {
    try {
      const body = payload?.body;
      if (typeof body === 'string') return true;
      warn(`[journal-publisher] ${method} refused a non-string text body (${body === null ? 'null' : typeof body})`);
    } catch {
      // Deliberately no detail from the thrown value: formatting it can throw too.
      warn(`[journal-publisher] ${method} refused an unreadable text body`);
    }
    return false;
  }
