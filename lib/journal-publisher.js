...
export function createJournalPublisher({
  ...
  // Inbound `error` control frames are logged once per distinct code, not
  // once per occurrence. A read_marker sent to a server that hasn't yet
  // landed agent read_marker support (matron-journal PR #2) rejects every
  // single one with the same 'forbidden' code, and markRead is called after
  // every user-mirrored publish — without this dedup that's an unbounded
  // stream of identical warnings for a condition the first warning already
  // fully describes.
