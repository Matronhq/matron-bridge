...
export function createJournalPublisher({
  ...
  // `{code, ref, detail}`, after the existing warn-once logging. Lets the
  // agent-chat invite manager correlate a rejected room op (ref:
  // 'agent_invite' etc.) back to its waiting tool call. Wrapped in
  ...
  function connect() {
    ...
    socket.on('message', (data) => {
      ...
      if (msg.op === 'hello_ok') {
        ...
        if (onOpError) {
          try { onOpError({ code: msg.code, ref: msg.ref, detail: msg.detail }); } catch { /* handler's problem, not the socket's */ }
        }
  ...
  return {
    ...
    upsertConvo(convoId, opts) {
      ...
      try {
        const { title, sessionState, parentConvoId, summary } = opts || {};
