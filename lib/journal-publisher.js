...
export function createJournalPublisher({
  ...
  //
  // `retain` (default true) is the right behavior for a TERMINAL repair frame
  // (subagent-reconcile's `done`): re-admitting it later is idempotent. For a
  // caller that re-offers NON-TERMINAL state (run-state-repair re-offers a live
  // convo's `running`/`waiting`), retention is actively wrong — drainPendingRepairs
  // appends at the queue TAIL, so a held snapshot can land AFTER a newer ordinary
  // update of the same convo and roll the row backwards. Such a caller passes
  // retain:false and recomputes from live state on the next capacity/reconnect tick.
  ...
  return {
    ...
    upsertConvoBestEffort(convoId, opts, options) {
      try {
        ...
        const onDelivered = typeof options?.onDelivered === 'function' ? options.onDelivered : undefined;
        // retain:false for a caller that re-offers non-terminal state, so a
        // refused offer is not held as a snapshot that could later land on top
        // of a newer transition (see enqueueBestEffort). Defaults to retention.
        const retain = options?.retain !== false;
        return enqueueBestEffort(frame, onDelivered, { retain });
      } catch (e) {
