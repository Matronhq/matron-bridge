...
export function bracketedPaste(text) {
  // Strip any embedded paste-terminator so message content can never close
  // the paste early and have its remainder interpreted by the TUI as raw
  // keystrokes / escape sequences.
  return `\x1b[200~${String(text).replaceAll('\x1b[201~', '')}\x1b[201~`;
}
