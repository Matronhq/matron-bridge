...
export function formatBox(box) {
  // Nothing dereferences `box` outside the try: a null/non-object element
  // in the journal's boxes array must take the degrade path too, not throw
  // past it and blank the listing this guard exists to protect.
  let name = 'unknown';
  let device = '?';
  try {
