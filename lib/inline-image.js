...
export async function prepareInlineImage(buffer, mime, overrides = {}) {
  ...
  // Passthrough/skip for a buffer sharp can't decode: the original is only
  // safe to inline when the API accepts its declared type and it's under the
  // hard byte cap. (HEIC lands here — prebuilt libvips has no HEIC decoder —
  // which is strictly better than today's behavior of sending image/heic and
  // failing the whole turn.)
  const undecodableFallback = () => {
  ...
  try {
    ...
    // header, exotic subformat): same safety rule as undecodable input.
    return undecodableFallback();
  }
}
