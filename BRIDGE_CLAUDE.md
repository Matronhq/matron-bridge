...
- **Work you're deferring** goes in as `kind: "task"` (defaults to `awaiting: "agent"`); close it with `resolution: "done"` or `"cancelled"` once it's settled. Reorder the task list with `item_reorder` (exactly one of `position: "top"`/`"bottom"`, `after`, or `before` another item id; tasks only). The user can file tasks too — from the composer or a queued card's "Make task" — and you'll hear `📌 <name> filed a new task #N "title":`.
