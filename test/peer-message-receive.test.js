  ...
  it('lists peer kinds including null and marks only the caller journal convo as self', async () => {
    ...
    const handlers = createAgentChatHandlers({
      ...
      publisher: {
        fetchRoster: async () => ({
          conversations: [
            { id: 'self-convo', title: 'Self', session_state: 'running' },
            { id: 'legacy-peer', title: 'Legacy', session_state: 'waiting' },
            { id: 'codex-peer', title: 'Codex', session_state: 'done' },
          ],
        }),
      },
      journalConvoIdFor: (value) => value.journalConvoId,
    ...
    assert.deepEqual(result, {
      ...
      body: {
        sessions: [
          { convo_id: 'self-convo', title: 'Self', session_state: 'running', is_self: true },
          { convo_id: 'legacy-peer', title: 'Legacy', session_state: 'waiting', is_self: false },
          { convo_id: 'codex-peer', title: 'Codex', session_state: 'done', is_self: false },
        ],
      },
    });
  });
