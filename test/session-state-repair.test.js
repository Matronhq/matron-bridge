...
describe('selectEpochRepairs', () => {
  ...
  it('RETIRES a stranded running convo with no live session — the permanent-Thinking case', () => {
    // REQUIRED CASE (2): a stranded `running` with no live session is retired.
...
describe('retirement settles against the RECORDED state (no publish loop)', () => {
  ...
  function sweep(outbox, liveConvoIds, sent) {
    ...
    const offer = (convoId, token, publish) => {
      ...
      outbox.settle(convoId, token); // stands in for the publisher's onDelivered callback
    };
...
describe('the transition publish does NOT settle on local send — reconcile is the authority', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'run-state-settle-'));
    file = join(dir, 'outbox.json');
  });
  afterEach(() => rmSync(dir, { recursive: true, force: true }));
  ...
  it('an evicted terminal `done` whose local send completed but whose server commit was LOST is re-offered next epoch', () => {
    // REQUIRED CASE (1): an evicted terminal `done` re-offered on the next connection epoch.
    const outbox = createRunStateOutbox({ file, log: silent });

    // journalSessionState write-aheads the terminal transition, then publishes `done`. It
    // deliberately does NOT settle on the publisher's onDelivered callback, because that callback
    // fires when the frame leaves the local socket buffer, NOT when the server persists it. Model
    // exactly that: note the transition, "publish" it, and do NOT settle.
    outbox.note('c1', 'done');
    // <-- the local send completed here (ws.send callback fired) but the connection dropped before
    //     the server committed. Settling right here strands the row forever; the write-ahead
    //     record MUST survive instead.
  ...
  it('a still-live convo whose transition send was lost is re-offered its current state on reconnect', () => {
    const outbox = createRunStateOutbox({ file, log: silent });
    outbox.note('c1', 'running');
    // No eager settle on local send. The record survives.
    expect(outbox.size()).toBe(1);
...
describe('index.js wiring', () => {
  ...
  it('write-ahead records to the DURABLE outbox but does NOT settle eagerly on local send', () => {
    ...
    // The transition publish must NOT clear the durable record on the publisher's local-send
    // callback — that callback fires when the frame leaves the local socket, not when the server
    // commits it, so settling there strands a lost-after-send `done` forever. The reconnect
    // reconciliation sweep (republishSessionStates) is the sole settle authority.
  ...
  it('still reconciles stranded subagents alongside it', () => {
