...
describe('createAgentChatHandlers', () => {
  ...
  describe('chatStart', () => {
    ...
    it('same-bridge target: arms inviteLocal BEFORE injecting the request, labels the room by convo title, and never calls invite()', async () => {
      ...
      expect(calls[0].opts.title).toBe(`🔗 [${chatRoomId.slice(0, 2)}] mac ↔ Local work — ci triage`);
    ...
    it('accepted: mints a room, upserts title, publishes opening message, records, invites — in that order', async () => {
      ...
      expect(calls[0]).toEqual({ call: 'upsertConvo', convoId: chatRoomId, opts: { title: `🔗 [${chatRoomId.slice(0, 2)}] mac ↔ dev-2 — ci triage`, sessionState: 'waiting' } });
      ...
      expect(calls[2].fields).toEqual({
        ...
        peerDeviceId: 7, peerName: 'dev-2', topic: 'ci triage', title: `🔗 [${chatRoomId.slice(0, 2)}] mac ↔ dev-2 — ci triage`,
      });
    ...
    it('omits the topic suffix from the title when no topic given', async () => {
      ...
      expect(calls[0].opts.title).toBe(`🔗 [${calls[0].convoId.slice(0, 2)}] mac ↔ dev-2`);
    });
    ...
    it('caps the title at 120 chars, link prefix included', async () => {
      ...
      expect(calls[0].opts.title.startsWith(`🔗 [${calls[0].convoId.slice(0, 2)}] mac ↔ dev-2 — xxx`)).toBe(true);
